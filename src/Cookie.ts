...
export const Cookie = /*#__PURE__*/createCookieInstance();

export function setBaseURL(url: string) {
    if (validUrl.isUri(url)) { webSite.url = new Url(url); }
}

export function createAccessor(url?: string) {
    if (url) { setBaseURL(url); }
    if (!storage.value) { storage.value = new CookieStorage(); }

    return {
        get: CookieStorage.prototype.getForUrl.bind(storage.value, false),
        set: CookieStorage.prototype.setForUrl.bind(storage.value, false),
    };
}
